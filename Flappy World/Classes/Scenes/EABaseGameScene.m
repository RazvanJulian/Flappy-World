...
- (void)didMoveToView:(SKView *)view
{
    ...
    self.topScoreLabel.text = [NSString stringWithFormat:@"Top: %lu", (unsigned long)self.topScores];
    
    self.scoreSound = [SKAction playSoundFileNamed:@"Checkpoint Sound.mp3" waitForCompletion:NO];
    self.crashSound = [SKAction playSoundFileNamed:@"Crush Sound.mp3" waitForCompletion:NO];
...
- (void)addHero
{
    self.hero = [EAHero spriteNodeWithImageNamed:@"bird1"];
...
- (void)addScoring
{
    ...
    self.scoresLabel.fontColor = [SKColor whiteColor];//yellowColor];
...
- (void)addTopScore
{
    ...
    self.topScoreLabel.fontSize = 15.0f;
    self.topScoreLabel.horizontalAlignmentMode = SKLabelHorizontalAlignmentModeCenter;
    self.topScoreLabel.fontColor = [SKColor whiteColor];//cyanColor];
    self.topScoreLabel.position = CGPointMake(self.size.width - 75.0f, self.size.height - 45.0f);
...
- (void)gameOver
{
    SKTransition *transition = [SKTransition fadeWithDuration:1.0f]; // doorsCloseHorizontalWithDuration:0.3f];
    ...
    if (_scores != scores) {
        ...
        if (self.topScoreBeated == NO) {
            if (_scores > self.topScores && self.topScores > 0) {
                [self runAction:[SKAction playSoundFileNamed:@"Checkpoint Sound.mp3"
